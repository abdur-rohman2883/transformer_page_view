...
class TransformerPageView extends StatefulWidget {
  ...
  static int? getRealIndexFromRenderIndex(
    ...
    int initPage = reverse ? ((itemCount ?? 0) - (index ?? 0) - 1) : index ?? 0;
    if (loop) {
...
class _TransformerPageViewState extends State<TransformerPageView> {
  ...
  Widget _buildItem(BuildContext context, int index) {
    TransformerPageController? controller = _pageController;
    ...
    return new AnimatedBuilder(
        ...
        builder: (BuildContext c, Widget? w) {
          ...
          if (widget.itemBuilder != null) {
            child = widget.itemBuilder?.call(context, renderIndex ?? 0);
          }
          if (child == null) {
            child = new Container();
          }
          ...
          if (_transformer?.reverse == true) {
            position = (page ?? 0) - index;
          } else {
            position = index - (page ?? 0);
          }
          ...
          return _transformer?.transform(child, info) ?? Container();
        });
  }

  double? _calcCurrentPixels() {
    _currentPixels =
        (_pageController?.getRenderIndexFromRealIndex(_activeIndex) ?? 0) *
  ...
  Widget build(BuildContext context) {
    ...
    Widget child = new PageView.builder(
      ...
      reverse: _pageController?.reverse ?? false,
    );
  ...
  void _onIndexChanged(int index) {
    ...
    ValueChanged<int>? onPageChanged = widget.onPageChanged;

    if (onPageChanged != null) {
      onPageChanged(_pageController?.getRenderIndexFromRealIndex(index) ?? 0);
    }
  }
    ...
    if (_pageController == null) {
      _pageController = new TransformerPageController(
          ...
          reverse: widget.transformer == null
              ? false
              : widget.transformer?.reverse ?? false);
    }
  ...
  void didUpdateWidget(TransformerPageView oldWidget) {
    ...
    if (_pageController?.getRenderIndexFromRealIndex(_activeIndex) != index) {
      _fromIndex = _activeIndex = _pageController?.initialPage;
      if (!created) {
        int initPage = _pageController?.getRealIndexFromRenderIndex(index) ?? 0;
  ...
  int? _calcNextIndex(bool next) {
    ...
    if (_pageController?.reverse == true) {
      if (next) {
        currentIndex--;
      } else {
        currentIndex++;
    ...
    if (_pageController?.loop == false) {
      if (currentIndex >= (_pageController?.itemCount ?? 0)) {
        currentIndex = 0;
      } else if (currentIndex < 0) {
        currentIndex = (_pageController?.itemCount ?? 0) - 1;
      }
    }
